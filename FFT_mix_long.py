...
for d in path.mix_day:
    for i in path.mix_subject:
        for j in path.mix_char2:
            for o in path.cutplusmix_time2:
                for l in path.time:
                    file_path = path.cut_plusmix_long + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "cut.CSV"
                    ...
                    for hm in range(60, 500, 60):
                        for p in range(1, 5):
                            if hm == 60:
                                hm -= 13
                            c_0 += c_f_ch0[(fq > (hm - p) - 1) & (fq < (hm - p) + 1)] + \
                                   c_f_ch0[(fq > (hm - p) - 1) & (fq < (hm - p) + 1)]
                            c_1 += c_f_ch1[ (fq > (hm - p) - 1) & (fq < (hm - p) + 1) ] + \
                                   c_f_ch1[(fq > (hm - p) - 1) & (fq < (hm - p) + 1) ]
                            c_2 += c_f_ch2[ (fq > (hm - p) - 1) & (fq < (hm - p) + 1) ] + \
                                   c_f_ch2[ (fq > (hm - p) - 1) & (fq < (hm - p) + 1) ]
                            c_3 += c_f_ch3[ (fq > (hm - p) - 1) & (fq < (hm - p) + 1) ] + \
                                   c_f_ch3[ (fq > (hm - p) - 1) & (fq < (hm - p) + 1) ]
                    ...
                    nf = open(path.fft_mix_7 + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "fft.CSV", 'w')
                    ...
                    new_data = new_data.iloc[ 1:257, 1:4]

                    new_data.to_csv(path.fft_mix_7 + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "fft.CSV")
