    ...
    for i in path.mix_subject:
        for j in path.mix_char:
            for o in path.cut8_time:
                for l in path.time:
                    file_path = path.cut8_mix + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "cut.CSV"
                    ...
                    # n_ch0 = hammingWindow * n_ch0.T
                    # n_ch1 = hammingWindow * n_ch1.T
                    # n_ch2 = hammingWindow * n_ch2.T
                    # n_ch3 = hammingWindow * n_ch3.T
                    # n_ch4 = hammingWindow * n_ch4.T
                    # n_ch5 = hammingWindow * n_ch5.T
                    # n_ch6 = hammingWindow * n_ch6.T
                    ...
                    c_f_ch0_ifft_real = np.convolve(c_f_ch0_ifft_real, b, mode="same")
                    c_f_ch1_ifft_real = np.convolve(c_f_ch1_ifft_real, b, mode="same")
                    c_f_ch2_ifft_real = np.convolve(c_f_ch2_ifft_real, b, mode="same")
                    c_f_ch3_ifft_real = np.convolve(c_f_ch3_ifft_real, b, mode="same")
                    c_f_ch4_ifft_real = np.convolve(c_f_ch4_ifft_real, b, mode="same")
                    c_f_ch5_ifft_real = np.convolve(c_f_ch5_ifft_real, b, mode="same")
                    c_f_ch6_ifft_real = np.convolve(c_f_ch6_ifft_real, b, mode="same")



                    nf = open(path.ifft_mix_8 + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "ifft.CSV", 'w')
                    ...
                    new_data = new_data.iloc[1:257, 1:7]

                    new_data.to_csv(path.ifft_mix_8 + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "ifft.CSV")
                    print("ifft/" + "/" + d + "/" + i + "/" + j  + "/" + l + "fft.CSV")

                    nf.close()

x2 = np.linspace(0,len(c_f_ch0_ifft_real),len(c_f_ch0_ifft_real))
plt.plot(x2, n_ch0)
plt.plot(x2, c_f_ch0_ifft_real, c="r", alpha=0.7, label='filtered')
