    ...
    for i in path.mix_subject:
        for j in path.mix_char2:
            for o in path.cutplusmix_time:
                for l in path.time:
                    file_path = path.cut_plusmix + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "cut.CSV"
                    ...
                    new_ch0 = np.abs(c_f_ch0_ifft_real)
                    new_ch1 = np.abs(c_f_ch1_ifft_real)
                    new_ch2 = np.abs(c_f_ch2_ifft_real)
                    new_ch3 = np.abs(c_f_ch3_ifft_real)



                    nf = open(path.ifft_mix_15 + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "ifft.CSV", 'w')
                    ...
                    c_f_ch0_p = pd.DataFrame(new_ch0)
                    c_f_ch1_p = pd.DataFrame(new_ch1)
                    c_f_ch2_p = pd.DataFrame(new_ch2)
                    c_f_ch3_p = pd.DataFrame(new_ch3)
                    # c_f_ch4_p = pd.DataFrame(c_f_ch4_ifft_real)
                    # c_f_ch5_p = pd.DataFrame(c_f_ch5_ifft_real)
                    # c_f_ch6_p = pd.DataFrame(c_f_ch6_ifft_real)

                    # データの切り出し
                    y1 = c_f_ch0_p.iloc[0:int(fft0), :]
                    y2 = c_f_ch1_p.iloc[0:int(fft1), :]
                    y3 = c_f_ch2_p.iloc[0:int(fft2), :]
                    y4 = c_f_ch3_p.iloc[0:int(fft3), :]
                    # y5 = c_f_ch4_p.iloc[0:int(fft4), :]
                    # y6 = c_f_ch5_p.iloc[0:int(fft5), :]
                    # y7 = c_f_ch6_p.iloc[0:int(fft6), :]
                    ...
                    new_data.to_csv(path.ifft_mix_15 + "/" + d + "/" + i + "/" + j + "/" + o + "/" + l + "ifft.CSV")
...
x2 = np.linspace(0,len(y1),len(y1))
plt.plot(x2, y1, c="r", alpha=0.7, label='filtered')
print(len(y1)/2)
plt.show()
