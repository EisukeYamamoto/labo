...
def plot_mixture_sources_predictions(X, original_sources, S, ch):
    ...
    if plot_flg == 1:
        if x_flg == 1:
            ...
            plotpath1 = path.png_ica_new + "/" + i + "/" + d + "/" + s + "/" + "infomax+fastICA" + "/各チャンネルの比較/" + str(
                ch) + "ch"
        elif x_flg == 2:
...
def write_plot(ch_list, name):
    ...
    if plot_flg == 1:
        if x_flg == 1:
            ...
            plotpath2 = path.png_ica_new + "/" + i + "/" + d + "/" + s + "/" + "infomax+fastICA" + "/" + name
        elif x_flg == 2:
...
nf_all = open(path.png_ica_new + "/" + "結果まとめ_infomax+fastICA.CSV", 'w', encoding="utf_8_sig")
