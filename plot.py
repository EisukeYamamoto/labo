...
def write_plot(ch_list, d, i, j, l, df):
    for n in range(0, 7):
        print("test/" + d + "/" + i + "/" + j + "/" + l + ".CSV/ch" + str(n+1))
        ...
        plt.plot(x, y)
        # plt.savefig(path.png + "/" + d + "/" + i + "/" + j + "/" + l + "/ch" + str(n + 1))
        plt.show()
        #4行2列の画像ができる
        #legendで凡例を消す
        ...
        if n == 6:
            df.plot(ylim=(-0.2,0.2),color="blue",subplots=True,layout=(4, 2),legend=False,
                    xticks=[0,500,1024],fontsize=10)
            plt.subplots_adjust(wspace=0.5, hspace=0.5)
            # plt.savefig(path.png + "/" + d + "/" + i + "/" + j + "/" + l + "/mu"+l)
            plt.show()
        plt.close('all')
...
def plot():
    ...
    for d in path.day:
        for i in path.subject:
            for j in path.char:
                for l in path.time:
                    file_path = path.fft + "/" + d + "/" + i + "/" + j + "/" + l + "fft.CSV"
